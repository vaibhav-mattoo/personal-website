...
The `/tasks` hub links to three areas: **Current tasks** (Fizzy at `tasks.vmattoo.dev`), **Paper reading queue** (placeholder), and **Calendar** (placeholder). Only the integration points live in this repo — a Caddy proxy block, the `vmattoo-shared` Docker network, and Kamal deploy templates under `deploy/fizzy/`. Fizzy itself is built and deployed from the user's fork.
