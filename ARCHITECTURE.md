...
| PDF toolkit (private) | **Stirling-PDF** (self-hosted, Spring Boot + Tesseract OCR), Compose service `stirling-pdf` on the default network, persisted via four named volumes (`stirling-configs`, `stirling-custom-files`, `stirling-logs`, `stirling-tessdata`). Caddy reverse-proxies `pdfs.vmattoo.dev` to it with `X-Robots-Tag: noindex` and a 256 MB request-body cap. Auth is Stirling's built-in login; initial admin credentials auto-generated by `deploy/bootstrap.sh` (printed once on first run, persisted in the VM's `.env`). | Not linked from the public site; subdomain only. |
| Public documents on `/notes/documents/` | Static PDFs in `site/public/documents/` listed via `site/src/config/public-documents.ts`. File sizes computed at build time. Served by Caddy as plain static files at `/documents/*.pdf` alongside the rest of the Astro build. No public link to Stirling from the listing; a subtle "Stirling login" link gives direct admin access from any device. | Config controls the listing, not URL access to files in `public/documents/`. |
...
Stirling-PDF data (configs, custom files, OCR tessdata, logs) lives in four Compose named volumes and is **destroyed by `deploy/teardown.sh`** alongside Waline's database — same documented contract. There are no automated backups; if you store PDFs you care about, add a periodic volume backup job (e.g. `docker run --rm -v personal-website_stirling-custom-files:/data …`) deliberately when that becomes important. Stirling-PDF (`pdfs.vmattoo.dev`) is the private workshop for preparing documents; the public exhibition is the static PDF directory served at `/documents/*.pdf` and listed on `/notes/documents/`.
